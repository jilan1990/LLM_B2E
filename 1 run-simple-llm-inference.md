...
接下来以GPT-2模型为例介绍decoder-only模型的结构，先画出总图（选用124M参数的模型），表示从"Never let your sense of morals prevent you from doing" 生成第一个单词"what"的过程。
...
V=50257：词表大小
d=768：隐藏维度（n_embd / d_model）
L=12：Transformer 层数
H=12：注意力头数
n ctx =1024：最大上下文token个数

------

### 1. 嵌入层（Embedding Stage）
...
- **词嵌入（Token Embeddings, WTE）**：将分词后的 ID 转换为高维向量（124M 模型中维度为 768），此处涉及参数 V × d = 50257 × 768 = 38,597,376。
- **位置嵌入（Positional Embeddings, WPE）**：由于 Transformer 无法感知顺序，GPT-2 使用一套可学习的向量，为每个位置（0 到 1023）赋予特定的坐标信息，此处涉及参数 nctx × d = 1024 × 768 = 786,432。
- **相加（Summation）**：将词向量与位置向量直接相加，得到携带顺序信息的初始语义向量。

**嵌入层总计**：38,597,376+786,432=39,383,808
...
#### A. 残差连接（Residual Connections）

- 每一层的结果都会加上该层的原始输入（x + Sublayer(x)）。这像是一条“高速公路”，允许信息不经损失地流向更深层，防止模型在堆叠过程中“迷失”。

#### B. 层归一化1（Pre-Layer Norm）
- GPT-2 将 LayerNorm 放在了注意力机制**之前**，这与原始 Transformer 不同，有助于稳定深层网络的训练，对应参数2d=2×768=1,536。

#### C. 多头自注意力子层（Masked Multi-Head Attention）

- **自注意力（Self-Attention）**：每个词生成各自的Q/K/V向量，用于作为后续分割为多头的输入，QKV 三个矩阵对应参数：3×d×d，此处代码实践中通过d* 3d的矩阵实现，768×(3×768)=1,769,472。
...
- 输出投影：对应代码中mha的c_proj参数，d×d=768×768=589,824

  自注意力层参数1,769,472+589,824=2,359,296

#### D. 层归一化2（Pre-Layer Norm）
- GPT-2 在前馈网络子层前也放了也放了一个归一化化层，这也与原始 Transformer 不同，有助于稳定深层网络的训练，对应参数2d=2×768=1,536。

#### E. 前馈网络子层（Feed-Forward Network, FFN）

- **维度扩展**：向量会先通过线性层扩大 4 倍（从 768 升到 3072），然后再压缩回来，对应参数d×4d+4d×d=2×d×4d=8d×d=

  8×768×768=8×589,824=4,718,592

- **GELU 激活函数**：相比 ReLU，GELU 更加平滑，有助于捕捉复杂的非线性特征。


单个Transformer 层总计参数1,536+2,359,296+1,536+4,718,592=7,080,960

所有 Transformer 层参数Transformer=L×Block=12×7,080,960=84,971,520
...
- **最终归一化（Final Layer Norm）**：对最后一层的输出进行标准化，对应参数2d=2×768=1,536。
...
总参数量（GPT-2 Small）Total = 嵌入层 + Transformer + Final LN = 39,383,808 + 84,971,520 + 1,536 = 124,356,864 ≈ 124M
