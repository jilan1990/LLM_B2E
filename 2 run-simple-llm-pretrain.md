...
- ![image-20260412160036488](./images/L_y_pred.png)是误差L对输出y_hat的梯度（预测得准不准)，此处的值为(y_hat - y_true)。

- ![image-20260412160247025](./images/y_pred_w2.png) 是输出y_hat对权重w2的梯度（权重对结果的影响)，此处的值为h.T。

  L对w2的倒数会在调整w2参数时候发挥作用。
...
这里误差像流水一样，从输出层反向流回隐藏层，L对w1的倒数会在调整w1参数时候发挥作用。

需要特别注意这个过程中使用了y_hat对h的求导，至此y_hat（h1*w21+h2*w22）对h和w分别都进行了求导。
...
计算图是**深度学习、自动微分、神经网络反向传播**的核心数学抽象，本质就是：

> **把复杂计算拆成一个个简单算子，用「有向图」表示数据流动和依赖关系**
...
1. **节点（Node）**
   - **数据节点**：变量、常数、张量（x,y,z,…）
   - **算子节点**：加减乘除、指数、对数、矩阵乘法等
2. **边（Edge）**
   - 有向边：表示**数据流向**，从输入 → 算子 → 输出
...
#### 3. 反向传播（Backward）核心原理

计算图最大价值：**自动求导（AutoDiff）**

反向传播本质：

> **从输出往输入，沿着图反向递推梯度，用链式法则**
...
**在图上就是：从 y 出发，反向走边，每条边乘上局部梯度，累乘到起点。**

#### 4. 两个关键性质

##### （1）局部性

每个算子只需要**自己的输入和输出**，就能算出：

- 正向结果
- 局部梯度 ∂out/∂in

不需要全局信息，**天然适合分布式、并行计算**。

##### （2）复用中间值

正向计算时保存所有中间节点（a,b,c…），反向直接用，**避免重复计算**。

这就是 PyTorch/TensorFlow 里的 **计算图缓存（ computation graph /tape）**。

#### 5. 静态图 vs 动态图

- **静态图（TensorFlow1、MXNet）**
...
类似编程语言，一般而言编译型侧重于运行速度，解释型相对会比较灵活，但也有一些兼顾两者优点的优化。

#### 6. 一句话总结

**计算图 = 用有向无环图表示计算 + 正向求值 + 反向自动链式求导**
